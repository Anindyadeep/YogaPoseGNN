...
class PoseRun(object):
    ...
    def load_model(self, model=None, model_path=None):
        ...
        if model is None and model_path is None:
            ...
            model_path = os.path.join(
                self.base_path, "saved_models/base_model.pth")
    ...
    def load_edge_index(self):
        ...
        """
        edge_index = YogaPosDataset(os.path.join(
            self.base_path, "Data/"), "train_data.csv").edge_index.to(self.device)
        return edge_index
    ...
    def _provide_video_writer_config(self, cap, save_as=None):
        ...
        file_to_save_as = os.path.join(BASEDIR, f'Video_results/result.avi') if save_as is None else os.path.join(BASEDIR, f'Video_results/{save_as}')
        vid_result = cv2.VideoWriter(file_to_save_as, cv2.VideoWriter_fourcc(*'MJPG'), 30, size)
        return vid_result

    def generate_results(self, cap, model = None, black_blackground = False):
        """
    ...
    def run_pose_on_webcam(self, cam_num = None, model = None, black_blackground = False):
        """
        ...
        for generated_frame in(self.generate_results(cap = cap, model = model, black_blackground = black_blackground)):
    ...
    def run_pose_on_video_source(self, source_file_path, model = None, black_blackground = False, save_capture_as = False):
        """
        ...
        video_result = self._provide_video_writer_config(cap, save_as = save_capture_file_name) if save_capture_as else None 

        for generated_frame in(self.generate_results(cap = cap, model = model, black_blackground = black_blackground)):
            cv2.imshow('Frame', generated_frame)
            if save_capture_as:
                video_result.write(generated_frame)
